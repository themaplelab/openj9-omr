...
AbsEnvStatic *AbsLoopAnalyzer::emptyEnv()
  ...
  //TODO: FIXME, There should be no NULL here...
  return new (getRegion()) AbsEnvStatic(getRegion(), _method, NULL);
  }
  ...
  if (_postState == nullptr)
    {
      ...
      _postState = new (region) AbsEnvStatic(region, _graph->_method, NULL);
      //TODO: FIXME: this will fail because of null above...
      _postState->getFrame()->interpret(_block);
      return true;
    }
  return false;
  }
...
AbsLoopAnalyzer::LoopHeader::interpret()
  ...
  firstPassState->getFrame()->interpret(_block);
  ...
  loopInvariant->getFrame()->interpret(_block);
